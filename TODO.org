* TODO easy
** shorthand for merge (M.unionWith const)
where if not present it gives the other one
** a way to restore a Zot to reasonable parameter values
** amplitude group for all drums, all pitched tracks
** ride cymbal
* bugs
** patterns are interfering in stacks
see j/bug2.hs and j/bug3.hs
* graph
** split rels v. reify
*** TODO how
**** add type: Reif
...
** TODO the proliferation of Museq sources (and payloads)
*** illustration
(l,time,time,Msg)
(time,Note)
etc.
*** maybe better: add Maybe to Note, and banish bare Msg values in a Museq
Stop using the Museq Msg type.
Instead use Museq Note',
where Note' is like Note but the synthdef is wrapped in a Maybe.
** can't insert silence into an abstract pattern generator
e.g.
seq d e f = [ (0, d)
            , (1, e)
            , (1.5, f) ]
