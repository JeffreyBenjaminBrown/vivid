...
module Montevideo.Monome.EdoMath (
    edoToFreq   -- ^ MonomeEdo -> EdoPitch -> Float
  , xyToEdo     -- ^ MonomeEdo -> (X,Y) -> EdoPitch
  ...
  , pcToXys     -- ^ MonomeEdo -> PitchClass -> (X,Y) -> [(X,Y)]
  , pcToLowXY   -- ^ MonomeEdo -> PitchClass -> (X,Y)
  , vv, hv      -- ^ MonomeEdo -> (X,Y)
  , modEdo      -- ^ Integral a => St EdoApp -> a -> a
  , modEdo_st   -- ^ Integral a => St EdoApp -> a -> a
  , pToPc       -- ^ MonomeEdo -> EdoPitch -> EdoPitchClass
...
pcToXys :: MonomeEdo -> (X,Y) -> EdoPitchClass -> [(X,Y)]
...
_enharmonicToXYs :: MonomeEdo -> (X,Y) -> [(X,Y)]
...
pcToLowXY :: MonomeEdo -> EdoPitchClass -> (X,Y)
pcToLowXY ec pc = let
  sk = _skip ec
  f :: X -> (X, Y)
  f x = let
...
modEdo :: Integral a => MonomeEdo -> a -> a
